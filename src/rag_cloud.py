...
def retrieve(soru: str, bolum=_AUTO) -> tuple[list[str], str | None]:
...
def _llm_stream(messages: list[dict], max_tokens: int = 1800, temperature: float = 0.2):
    if LLM_PROVIDER == "openrouter":
        client, model = _openrouter_client(), LLM_MODEL_OPENROUTER
    else:
        client, model = _groq_client(), LLM_MODEL_GROQ
...
# --------------------------------------------------------------------------- #
def respond(soru: str, bolum=_AUTO) -> dict:
    ...
    ctx, _ = retrieve(soru, bolum=bolum)
    return {"mode": "llm", "text": None, "token_iter": _llm_stream(_messages_for(soru, ctx, bolum)),
            "bolum": bolum, "n": len(ctx)}
